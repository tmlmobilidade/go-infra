...
# # #
# COMPUTE
# Single Nominatim VM with attached block volume for data.

resource "oci_core_instance" "nominatim" {
	...
	metadata = {
		...
		# cloud-init runs on first boot and configures Nominatim.
...
resource "oci_core_volume_attachment" "nominatim_data" {

	instance_id = oci_core_instance.nominatim.id
