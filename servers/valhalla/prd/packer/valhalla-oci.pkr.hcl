# -----------------------------------------------------------------------
# valhalla-oci.pkr.hcl
#
# Builds a custom OCI image for Valhalla with Docker and OS tuning
# pre-installed. Valhalla runs as a Docker container at runtime
# (image: ghcr.io/tmlmobilidade/valhalla-tml) so it can be updated
...
source "oracle-oci" "valhalla-source" {
...
build {
	...
	sources = ["source.oracle-oci.valhalla-source"]
	...
	provisioner "file" {
		source = "${path.root}/init/setup-valhalla.sh"
		destination = "/opt/app/setup-valhalla.sh"
	}

	provisioner "shell" {
		inline = [
			"sudo chmod +x /opt/app/setup-valhalla.sh"
		]
	}

	# Daily Portugal OSM refresh (invoked by cron from cloud-init).

	provisioner "file" {
		source = "${path.root}/init/update-portugal-osm.sh"
		destination = "/opt/app/update-portugal-osm.sh"
	}

	provisioner "shell" {
		inline = [
			"sudo chmod +x /opt/app/update-portugal-osm.sh"
		]
	}
	...
	# nginx reverse-proxy config (TLS front for Valhalla)
