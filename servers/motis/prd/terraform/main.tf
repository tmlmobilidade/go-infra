...
# # #
# COMPUTE
# Single Motis VM with attached block volume for tiles/PBF.

resource "oci_core_instance" "motis" {
	...
	metadata = {
		...
		# cloud-init runs on first boot and configures Motis.
...
# Volume is pre-created outside Terraform so destroy
# does not wipe tiles / PBF data.

resource "oci_core_volume_attachment" "motis_data" {

	instance_id = oci_core_instance.motis.id

	volume_id = var.block_volume_ocid
