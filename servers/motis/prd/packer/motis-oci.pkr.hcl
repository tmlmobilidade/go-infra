...
# motis-oci.pkr.hcl
#
# Builds a custom OCI image for Motis with Docker and OS tuning
# pre-installed. Motis runs as a Docker container at runtime
# (image: ghcr.io/tmlmobilidade/motis-tml) so it can be updated
...
source "oracle-oci" "motis-source" {
...
build {
	...
	sources = ["source.oracle-oci.motis-source"]
	...
	provisioner "file" {
		source = "${path.root}/init/setup-motis.sh"
		destination = "/opt/app/setup-motis.sh"
	}

	provisioner "shell" {
		inline = [
			"sudo chmod +x /opt/app/setup-motis.sh"
		]
	}
	...
	provisioner "file" {
		source = "${path.root}/init/update-motis.sh"
		destination = "/opt/app/update-motis.sh"
	}

	provisioner "shell" {
		inline = [
			"sudo chmod +x /opt/app/update-motis.sh"
		]
	}
	...
	provisioner "file" {
		source = "${path.root}/init/config.yml"
		destination = "/opt/app/config.yml"
	}

	# nginx reverse-proxy config (TLS front for Motis)
