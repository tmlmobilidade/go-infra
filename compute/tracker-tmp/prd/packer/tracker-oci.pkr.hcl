...
# tracker-oci.pkr.hcl
#
# Builds a custom OCI image for Tracker nodes with Docker and
...
source "oracle-oci" "tracker_base" {
...
build {
	...
	sources = ["source.oracle-oci.tracker_base"]
	...
	provisioner "shell" {
		inline = [
			...
			"sudo chown ubuntu:ubuntu /opt/app/secrets"
		]
	}

	# The compose.yaml file holds the configuration
	# that defines the Tracker container and its settings.
