...
# grafana-oci.pkr.hcl
#
# Builds a custom OCI image for Grafana with Docker and OS tuning
# pre-installed. Grafana runs as a Docker container at runtime
# (image: ghcr.io/tmlmobilidade/grafana) so it can be updated
...
source "oracle-oci" "grafana-source" {
...
build {
	...
	sources = ["source.oracle-oci.grafana-source"]
