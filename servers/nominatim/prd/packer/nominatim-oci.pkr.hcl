# -----------------------------------------------------------------------
# nominatim-oci.pkr.hcl
#
# Builds a custom OCI image for Nominatim with Docker and OS tuning
# pre-installed. Nominatim runs as a Docker container at runtime
# (image: ghcr.io/tmlmobilidade/nominatim) so it can be updated
...
source "oracle-oci" "nominatim-source" {
...
build {
	...
	sources = ["source.oracle-oci.nominatim-source"]
	...
	# nginx reverse-proxy config (TLS front for Nominatim)
