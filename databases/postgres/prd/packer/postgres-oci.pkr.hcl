# -----------------------------------------------------------------------
# postgres-oci.pkr.hcl
#
# Builds a custom OCI image for postgres nodes with Docker and
# OS tuning pre-installed. postgres itself runs as a Docker container
...
source "oracle-oci" "postgres-source" {
...
build {
	...
	sources = ["source.oracle-oci.postgres-source"]
	...
	# that defines the postgres container and its settings.
