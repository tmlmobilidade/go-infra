# -----------------------------------------------------------------------
# tts-server-oci.pkr.hcl
#
# Builds a custom OCI image for TTS Server with Docker and OS tuning
# pre-installed. TTS Server runs as a Docker container at runtime
# (image: ghcr.io/tmlmobilidade/tts-server) so it can be updated
...
source "oracle-oci" "tts-server-source" {
...
build {
	...
	sources = ["source.oracle-oci.tts-server-source"]
	...
	# nginx reverse-proxy config (TLS front for TTS Server)

	provisioner "shell" {
		inline = [
			"mkdir -p /opt/app/nginx/www"
		]
	}
	...
	provisioner "file" {
		source = "${path.root}/init/nginx/www/index.html"
		destination = "/opt/app/nginx/www/index.html"
	}
